...
struct HashType
{
	struct RecordType* pRecord; //Create a pointer to RecordType
};
...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
	...
	for(int i = 0; i < tableSize; i++)
	{
		...
		struct RecordType* temp = hashTable[i].pRecord;
	if (temp!= NULL){
		while(temp!= NULL)
		{
...
int main(void)
{
	...
	struct HashType* hashTable = NULL; // Initialize the hash table
    
	hashTable = (struct HashType*)calloc(sizeof(struct HashType), hashtableSize); // initialize a hashTable, use calloc (so everything is assigned to NULL)
